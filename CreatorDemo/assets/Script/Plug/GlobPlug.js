window.DE_DEBUG = true
window.DE_VISIBLESIZE = cc.view.getVisibleSize()
window.DE_DESIGNRESOLUTIONSIZE = cc.view.getDesignResolutionSize()

window.deLog = function (params) {
    if (DE_DEBUG) {
        if (typeof(params) === 'object') {
            console.log(`[SYS_DEBUG]${JSON.stringify(params)}`)
        } else {
            console.log(`[SYS_DEBUG]${params}`)
        }
    } else {
        return
    }
